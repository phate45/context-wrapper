...
`),[])}function wt(r){if(!r.startsWith("---"))return r;let e=r.indexOf(`
---`,3);return e===-1?r:r.slice(e+4).replace(/^\n+/,"")}function Ot(r,e){let t=e.match(/^(\d{4}-\d{2}-\d{2})\.md$/);if(!t)return r;let n=t[1],s=r.split(`
...
To search only these files: \`search(queries: [...], source: "${b}")\``;return{content:[{type:"text",text:$e+Be+je}]}}let D=f;if(_==="search"&&f?.queries!==void 0){let S=Q(f.queries,"queries");if(!S.ok)return{content:[{type:"text",text:`Invalid arguments for search:
  - ${S.message}`}],isError:!0};D={...f,queries:S.value}}let C;if(_==="execute"&&f?.path!==void 0?C="ctx_execute_file":C=Ue[_],!C)return{content:[{type:"text",text:`Unknown tool: ${_}`}],isError:!0};let A=await a.callTool({name:C,arguments:D});return Oe(A),_==="search"&&e?.config.searchReminder!==void 0&&Ie(A,e.config.searchReminder),A});let p=new Vt;await g.connect(p),process.stderr.write(`[context-wrapper] MCP server ready (${d.length} tools) [tmp=${t}]
