...
---
**Batch ID:** \`${_}\`
To search only these files: \`search(queries: [...], source: "${_}")\``;return{content:[{type:"text",text:v+k+N}]}}let y;if(p==="execute"&&f?.path!==void 0?y="ctx_execute_file":y=Ce[p],!y)return{content:[{type:"text",text:`Unknown tool: ${p}`}],isError:!0};let w=await a.callTool({name:y,arguments:f});if(p==="search"&&e?.config.searchReminder!==void 0){let b=e.config.searchReminder,S=w.content;if(Array.isArray(S))for(let _ of S){if(_.type!=="text"||typeof _.text!="string")continue;let T=/\n\n⚠ search call #\d+\/\d+ in this window\..+$/s,D=/^BLOCKED: \d+ search calls in \d+s\..+$/s;T.test(_.text)?_.text=b===!1?_.text.replace(T,""):_.text.replace(T,`

${b}`):D.test(_.text)&&(_.text=b===!1?"":String(b))}}return w});let E=new Nt;await h.connect(E),process.stderr.write(`[context-wrapper] MCP server ready (${g.length} tools) [tmp=${t}]
`);let m=async()=>{await Promise.allSettled([a.close(),h.close()])};process.stdin.on("end",()=>process.exit(0)),process.on("SIGINT",async()=>{await m(),process.exit(0)}),process.on("SIGTERM",async()=>{await m(),process.exit(0)}),process.on("exit",()=>{try{process.kill(u)}catch{}try{kt(t,{recursive:!0,force:!0})}catch{}})}Ut().catch(s=>{process.stderr.write(`[context-wrapper] Fatal: ${s.message}
${s.stack}
`),process.exit(1)});
